...
class PhysRopeConstraint(): TwoShipsMConstraint(), VEAutoSerializable {
    ...
    private fun makeLinkData(level: ServerLevel, pos: Vector3d, rot: Quaterniond, mass: Double, length: Double, radius: Double): PhysicsEntityData {
        ...
        return PhysicsEntityData(
            ...
            BodyTransformFactory.create(pos.toJomlVector3d(), rot, Vector3d().toJomlVector3d(), Vector3d().toJomlVector3d()),
...
//        return true
        ...
        entities.withIndex().forEach { (i, entity) ->
            ...
            cIDs.add(level.shipObjectWorld.createNewConstraint(VSDistanceJoint(
                prevId,    VSJointPose(prevPos.toJomlVector3d(), Quaterniond()),
                entity.id, VSJointPose((dir * (length + radius)).toJomlVector3d(), Quaterniond()),
                minDistance = 0f,
                maxDistance = 0.01f
            )) ?: run { return false })
